...
function! RunSpec()
    ...
    :echo g:speckyRunSpecCmd 
	" Call specs and gather up the output
	"
	:echo "-kkkk-------------------sd-------------------------------------------_____"
	let l:spec   = bufname('%')
    "let l:spec = expand("%:p")
"	let lineNumber = RegSearch("/Szenario/","b")		
	let l:cmd    =  g:speckyRunSpecCmd . ' ' . l:spec
"	if lineNumber > 3
"	let l:cmd = l:cmd.":".lineNumber
"	endif
	:echo l:cmd
...
function! RegSearch(regexp,direction)
