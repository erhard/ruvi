...
require File.expand_path(File.dirname(__FILE__) + "/lang_utils")
...
class VimLocalHistory::Repository
  ...
  def initialized?
    ...
    File.exists? "#{File.expand_path(location)}/.git"
  end
  ...
  def check_enabled
    ...
    expanded_dir_name = File.expand_path(location)
    if File.directory?(expanded_dir_name)
      return true
    else
      print ("no valid Dir " + expanded_dir_name)
      return false

      ####For this Feature is not yet a test present############
      #  Dir.mkdir(expanded_dir_name)
    end
  end
